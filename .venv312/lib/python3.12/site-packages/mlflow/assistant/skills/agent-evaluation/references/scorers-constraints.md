...
- DONE Tool selection/usage
- DONE Execution flow
- DONE Intermediate reasoning
- DONE Multi-step processes

Use `{{inputs}}`/`{{outputs}}` when evaluating:

- DONE Final input/output quality only
- DONE Response relevance
- DONE Answer correctness
...
```bash
-i "Evaluate if {{ outputs }} is high quality. Return yes or no."  # DONE Has variable
```
...
```bash
# DONE Correct - has variable, uses yes/no, correct parameters
uv run mlflow scorers register-llm-judge \
  -n "RelevanceCheck" \
...
```bash
# DONE Correct - uses {{trace}} only (no other variables), yes/no, correct parameters
uv run mlflow scorers register-llm-judge \
  -n "ToolUsageCheck" \
